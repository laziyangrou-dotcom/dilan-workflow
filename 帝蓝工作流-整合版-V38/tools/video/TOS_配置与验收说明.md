# Seedance 2.0 参考视频 · 火山 TOS 配置与验收说明（V41）
...
| 6 | 图片/音频参考（V41 起） | 启用 TOS 时图片、音频参考也自动上传 TOS 并以预签名 URL 传给 Ark（按内容 sha256 去重，同一文件只上传一次）；未启用 TOS 才回退内嵌 `data:`。几十 MB 的 base64 请求体会被 Ark 网关直接断开（WinError 10054），故建议保持 TOS 启用 |
