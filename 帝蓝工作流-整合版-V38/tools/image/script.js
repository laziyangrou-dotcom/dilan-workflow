...
const TOOL_PATH='image', MODULE_LABEL='分镜', MODULE_CHILD_LABEL='分镜子项目';
...
async function submitMessage(shotId,tabId){if(!assertEditShot(shotId))return;const viewState=captureViewState();const msg=(drafts[tabId]||'').trim();const mode=isStartImageText(msg)?'generate':'chat';pendingState[tabId]=mode==='chat'?'正在与 GPT 聊天…':'正在生成图片…';renderScenes(viewState.prompt);restoreViewState(viewState);const base=selectedBase[tabId]||null;const redoId=redoState[tabId]?.messageId||null;try{await saveProject();const r=await api('/api/submit',{method:'POST',body:{project:currentProject,shot_id:shotId,tab_id:tabId,message:msg,mode,base_image_id:base,redo_message_id:redoId,api_key:$('apiKey').value.trim(),user_name:currentUserName}});preservePromptDraftForTab(tabId,msg,r.data);delete selectedBase[tabId];delete redoState[tabId];delete pendingState[tabId];updateFromServer(r,viewState);term(mode==='chat'?'聊天完成':'生成完成')}catch(e){delete pendingState[tabId];renderScenes(viewState.prompt);restoreViewState(viewState);toast(e.message);term('失败：'+e.message)}}
