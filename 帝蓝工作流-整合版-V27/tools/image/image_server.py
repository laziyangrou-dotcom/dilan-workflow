...
# -*- coding: utf-8 -*-
"""
Image Storyboard Tool - parent/child projects + SC tabs edition.
...
PROJECT_MODULE_TYPE = "image"
PROJECT_MODULE_LABEL = "分镜模块"
...
DEFAULT_CONFIG = {
    "global_defaults": {
        ...
        "output_rule": "output/image/{project}/{scene_code}/{shot_id}/",
...
def save_config(cfg):
    ...
    merged["global_defaults"]["output_rule"] = "output/image/{project}/{scene_code}/{shot_id}/"
...
def decide_operation(thread, user_text, explicit_mode, selected_base_id):
    ...
    if current:
        # 用“恢复后的真实需求”判断是否为修改：用户可能先提“消除书包”，再单独发“开始生图”，
        # 此时 user_text 只是触发词，必须回看本标签页最近的真实需求，否则会误判为重新生成而丢掉基准图。
        ...
        if any(w in effective for w in edit_words):
            return "edit", current
        # 若本次只是触发词、且上一条真实需求存在但未命中关键词，仍默认在当前基准图上继续修改，
        # 避免“纯触发词”被当成无图的重新生成。
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def api_storyboard_upload(self):
        ...
        out.write_bytes(raw)
        file_url = "/output/image/" + out.relative_to(OUTPUT_IMAGE_DIR).as_posix()
        img = {"image_id": new_id("img"), "file_path": file_url, "created_at": now_str(), "operation": "manual_storyboard", "user_message": name, "prompt": "手动添加分镜图", "settings": dict(tab.get("settings") or {}), "tab_id": tab.get("tab_id")}
...
if __name__ == "__main__":
    print(f"Image storyboard tool running at http://localhost:{PORT}")
    print(f"Root: {ROOT}")
